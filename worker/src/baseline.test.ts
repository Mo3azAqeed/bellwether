...
const FAKE_ENV = {} as Env; // computeHealth only threads env through to the (mocked) PostHog call
...
describe("computeHealth", () => {
  it("tags a flat-usage account as stable", async () => {
    ...
    const health = await computeHealth(FAKE_ENV, account(), NOW);
  ...
  it("computes renewal days out from the account's renewal date", async () => {
    vi.mocked(getDailyActiveSeats).mockResolvedValue(points(60, 15));
    const health = await computeHealth(FAKE_ENV, account({ renewal_date: "2026-09-20" }), NOW);
